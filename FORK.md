...
Feature pages: `docs/spec/90-fork/`. Generated against `upstream/main` = `cc06bf4d3`; regenerate the raw list with the command at the end.
...
| `apps/desktop/test/agent-capability-settings.test.mjs` | +4 −1 | hook | feat: 引入子代理深度、子代理工具调用选择 |
...
| `packages/agent-runtime/src/runtime.ts` | +286 −47 | integration point | refactor(fork): isolate subagent-depth into fork-owned modules |
...
| `scripts/e2e-transcript-render.mjs` | +2 −0 | hook | fix(e2e): adapt desktop scenarios to upstream UI changes |
| `scripts/e2e/transcript-render.tsx` | +139 −4 | integration point | fix(e2e): adapt desktop scenarios to upstream UI changes |
...
| upstream file | +/− vs upstream | kind | introduced by |
| --- | --- | --- | --- |
| `docs/.vitepress/config.mts` | +1 −0 | hook | this PR / merge resolution |
| `docs/spec/03-runtime/02-agent-runtime.md` | +2 −0 | hook | feat(subagents): discover trusted extension tools dynamically |
| `docs/spec/04-ux/08-component-spec.md` | +1 −0 | hook | chore(voice): follow upstream for voice docs and styles |
| `docs/spec/NAV.md` | +8 −0 | hook | chore(voice): follow upstream for voice docs and styles |
| `docs/zh-CN/spec/NAV.md` | +8 −0 | hook | chore(voice): follow upstream for voice docs and styles |
...
| `apps/desktop/test/rpc-lifecycle-contract.test.mjs` | +1 −1 | hook | this PR / merge resolution |
...
| `crates/host-core/src/sessions/fork_files.rs` | +6 −1 | hook | this PR / merge resolution |
...
| `packages/agent-runtime/src/hosted-search-compaction.test.ts` | +1 −1 | hook | this PR / merge resolution |
...
Total upstream files touched: 95.

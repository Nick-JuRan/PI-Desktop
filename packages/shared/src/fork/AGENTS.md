...
Scope: shared cross-process contracts, constants, and validators for fork features. This directory is an **island** of the fork
...
- Export everything through `index.ts`, published as `@pi-desktop/shared/fork`.
  Consumers import fork symbols from that subpath only; nothing here is
  re-exported from the upstream `@pi-desktop/shared` barrel.
- Keep this package free of desktop or Node-specific code (root `AGENTS.md`
  §4); constants defined here are the single source of truth that host-core
  mirrors with a comment pointing back.

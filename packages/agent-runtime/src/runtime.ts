...
Do not invent objections or turn speculative risks into blockers. Stop when the requested work is complete and relevant checks pass, or report a genuine blocker.`),
  ...
  private buildSubagentTool(scope: DelegationScope = ROOT_DELEGATION_SCOPE): AgentTool {
    ...
    return {
      ...
      execute: async (toolCallId, params) => {
        ...
        if (resume) {
          const current = this.resolveResumeChain(resume, definition.name, scope.ownerDelegationId);
  ...
  private settleDelegation(
    ...
    // Resolve before event publication or bookkeeping so side effects cannot
    // strand TaskWait or the parent's idle auto-resume.

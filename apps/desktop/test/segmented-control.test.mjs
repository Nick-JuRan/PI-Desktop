...
test("segmented controls preserve selection semantics and stable tab-panel links", async () => {
  ...
  try {
    ...
    assert.match(selected, /role="tab"(?=[^>]*id="host-pair")(?=[^>]*aria-controls="panel-pair")(?=[^>]*aria-selected="true")[^>]*>/);
    assert.match(selected, /role="tab"(?=[^>]*id="host-ssh")(?=[^>]*aria-controls="panel-ssh")(?=[^>]*aria-selected="false")[^>]*>/);
